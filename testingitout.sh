...
do
	...
	select TOPPING in "Meat-Lovers Pizza" "Veggie Pizza" "Cheese Pizza" "Pineapple Pizza" "Sausage Pizza" "Pepperoni Pizza"
	do
		...
		echo "This is your current order.`echo $'\n'`"
		echo "${pizza_list[@]}"
		read -p "Would you like to place another order?" add_order
		if [[ $add_order == "Yes" ]];then
			order_pizza
		else
			echo " I haven't gotten this far in the code yet :( "
		fi
done
